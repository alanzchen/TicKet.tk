...
{
关于输入的格式:
如果只有一天的数据，则为"ExeName 1day %八位数日期% %第一天数据库路径% %写出数据库路径%"
如果只有两天的数据，则为"ExeName 2day %八位数日期% %第一天数据库路径% %第二天数据库路径% %写出数据库路径%"
}
...
begin
  ...
  DataDate:=StrToInt(ParamStr(2));   //取数据日期
  //取几天的数据库的文件路径
  Day1DB:=ParamStr(3);
  Day2DB:=ParamStr(4);
  ...
  Case day of
    1:begin
        Day1DBText:=Day1DB;
        OutputDB:=Day2DB;                       //只有一天时，第四个参数为输出的数据库名字
        TotalTime:=day*24*2;
    ...
    2:begin
        ...
        OutputDB:=ParamStr(5);                 //只有两天时，第五个参数为输出的数据库名字
        TotalTime:=day*24*2;
        for id:=1 to RouterNumber do
        begin
          EcptJump:=false;
